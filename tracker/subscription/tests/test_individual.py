...
class TestIndividualSubscription(TestCase):
    def setUp(self):
        ...
        self.individual = CoreUser.objects.create_core_user_from_web({
    ...
    def test_create_individual_subscription(self):
        subscription_data = IndividualSubscriptionData.objects.create(created_by=self.system_user, core_user=self.individual, individual_subscription_type=self.subscription_type, expiration_date=timezone.now() + timezone.timedelta(days=self.subscription_type.current.term_length_days), expired=False)
        subscription = IndividualSubscription.objects.create(created_by=self.system_user, current=subscription_data)
        subscription_data.individual_subscription = subscription
        subscription_data.save()
        subscription.set_expiration_date(self.individual.id, self.subscription_type)
    ...
    def test_expire_individual_subscription(self):
        subscription_data1 = IndividualSubscriptionData.objects.create(created_by=self.system_user, core_user=self.individual, individual_subscription_type=self.subscription_type, expiration_date=timezone.now() + timezone.timedelta(days=self.subscription_type.current.term_length_days), expired=False)
        subscription1 = IndividualSubscription.objects.create(created_by=self.system_user, current=subscription_data1)
        subscription_data1.individual_subscription = subscription1
    ...
    def test_expire_individual_subscription_already_expired(self):
        subscription_data1 = IndividualSubscriptionData.objects.create(created_by=self.system_user, core_user=self.individual, individual_subscription_type=self.subscription_type, expiration_date=timezone.now() + timezone.timedelta(days=self.subscription_type.current.term_length_days), expired=True)
        subscription1 = IndividualSubscription.objects.create(created_by=self.system_user, current=subscription_data1)
        subscription1.current.individual_subscription = subscription1
