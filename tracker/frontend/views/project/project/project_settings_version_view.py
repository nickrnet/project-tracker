...
class ProjectSettingsVersionView(LoginRequiredMixin, View):
    ...
    def post(self, request, project_id, version_id):
        ...
        if logged_in_user not in project.users.all():
            messages.error(request, 'The specified Version does not exist or you do not have permission to see it.')
            return redirect("/login?next=/projects/")
