...
class TestIssueModalView(TestCase):
    def setUp(self):
        """
    ...
    def test_issue_modal_view_redirects_when_not_logged_in(self):
        response = self.http_client.get(reverse('issue_modal', kwargs={'issue_id': str(self.issue1.id)}))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, '/login?next=/issue/' + str(self.issue1.id) + '/modal/')
    ...
    def test_issue_modal_view_get_no_project_permission(self):
    ...
    def test_issue_modal_view_get_issue_does_not_exist(self):
        self.http_client.force_login(user=self.user1.user)
        response = self.http_client.get(reverse('issue_modal', kwargs={'issue_id': '845d1553-6709-4c26-bb65-051da7de8d5c'}))
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, '/issues')
        self.assertIn('The specified issue does not exist.', str(messages))
    ...
    def test_issue_modal_view_post(self):
        ...
        response = self.http_client.post(
            reverse('issue_modal', kwargs={'issue_id': str(self.issue1.id)}),
            data={
    ...
    def test_issue_modal_view_post_issue_does_not_exist(self):
        self.http_client.force_login(user=self.user1.user)
        response = self.http_client.post(
            reverse('issue_modal', kwargs={'issue_id': 'f4c72cd7-c27f-4894-99db-eb6d249eb651'}),
            data={
                'project': str(self.project1.id),
                'summary': 'Updated Issue 1 Summary',
                'description': 'Updated description for issue 1',
    ...
    def test_issue_modal_view_post_no_project(self):
        self.http_client.force_login(user=self.user1.user)
        response = self.http_client.post(
            reverse('issue_modal', kwargs={'issue_id': str(self.issue1.id)}),
            data={
